...
def test(batch_size=16,
    ...
    checkpoint = torch.load('/u/home/javadov/ai_kit/ai-student-starter-kit/checkpoints/melornotdata_resnet50.ckpt')
    ...
    for i in range(5):
        for j in range(5):
            ...
            torch_img = transforms.Compose([transforms.Resize((224, 224)),transforms.ToTensor()])(pil_img)
            normed_torch_img = transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])(torch_img)[None]
            gradcam = GradCAMpp.from_config(model_type='vgg', arch=model, layer_name='backbone.layer4')
            mask, logit = gradcam(normed_torch_img, class_idx=None) 
...
def train(batch_size=16,
    ...
    early_stopping=False #flag
    model = SimpleClassifier(model_name='vgg16',learning_rate=learning_rate,
                            class_weights=list(dataset.class_weights),
                            num_classes=dataset.num_classes,
                            check_early_stop=early_stopping)
...
#nvidia smi
    ...
    trainer.save_checkpoint('/u/home/javadov/ai_kit/ai-student-starter-kit/checkpoints/Isicdata_vgg16_300.ckpt') #resnet50, max100epoch
def main():
    #seed
